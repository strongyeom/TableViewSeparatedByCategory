...
class TVTableViewCell : UITableViewCell {
    ...
    let descriptionLabel = {
       let label = UILabel()
        print("desRC2 -1 ", CFGetRetainCount(label))
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        configureView()
        print("RC",CFGetRetainCount(titleLabel))
        print("desRC3 - 2", CFGetRetainCount(descriptionLabel))
    ...
    func configureView() {
        [mediaImageView, titleLabel, descriptionLabel].forEach {
            self.addSubview($0)
            print("테이블 뷰 RC - \(CFGetRetainCount(self))")
        }
    }
